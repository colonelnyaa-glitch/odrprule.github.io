(() => {
  ...
  function getConfig(){return{owner:sessionStorage.getItem('odrp_owner')||$('#ghOwner').value.trim(),repo:sessionStorage.getItem('odrp_repo')||$('#ghRepo').value.trim(),branch:sessionStorage.getItem('odrp_branch')||$('#ghBranch').value.trim()||'main',token:sessionStorage.getItem('odrp_token')||$('#ghToken').value.trim()}}
  function saveConfig(){sessionStorage.setItem('odrp_owner',$('#ghOwner').value.trim());sessionStorage.setItem('odrp_repo',$('#ghRepo').value.trim());sessionStorage.setItem('odrp_branch',$('#ghBranch').value.trim());sessionStorage.setItem('odrp_token',$('#ghToken').value.trim())}
  function loadConfig(){['owner','repo','branch','token'].forEach(k=>{const v=sessionStorage.getItem('odrp_'+k);const id={owner:'#ghOwner',repo:'#ghRepo',branch:'#ghBranch',token:'#ghToken'}[k];if(v)$(id).value=v})}
